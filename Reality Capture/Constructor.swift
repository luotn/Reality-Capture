...
class Constructor:ObservableObject {
    ...
    func process(inputFolder: String, outputFilename: String, ordering: String, sensitivity: String, PhotogrammetryView: PhotogrammetryView) throws {
        ...
        let request = PhotogrammetrySession.Request.modelFile(url: URL(fileURLWithPath: outputFilename), detail: Request.Detail.reduced)
            ...
            do {
                for try await output in session.outputs {
                    ...
                    case .requestError(let request, let error):
                        await PhotogrammetryView.updateProgress(fractionComplete: 0.0)
                        throw IllegalOption.requestError("Request \(String(describing: request)) had an error: \(String(describing: error))")
                    case .requestComplete(let request, let result):
                        Constructor.handleRequestComplete(request: request, result: result, PhotogrammetryView: PhotogrammetryView)
                    case .requestProgress(_, let fractionComplete):
                        await PhotogrammetryView.updateProgress(fractionComplete: fractionComplete)
    ...
    private static func handleRequestComplete(request: PhotogrammetrySession.Request,
                                              ...
                                              PhotogrammetryView: PhotogrammetryView) {
        ...
        PhotogrammetryView.compeleted = true
    }
