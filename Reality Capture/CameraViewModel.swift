...
private let logger = Logger(subsystem: "com.TianningDigital.Reality-Capture",
...
class CameraViewModel: ObservableObject {
    ...
    private func capturePhotoAndMetadata() {
        ...
        /// the main thread.
        let videoPreviewLayerOrientation = session.connections[0].videoRotationAngle

        sessionQueue.async {
            if let photoOutputConnection = self.photoOutput.connection(with: .video) {
                photoOutputConnection.videoRotationAngle = videoPreviewLayerOrientation
            }
            ...
            // Turn on depth data, and quality prioritzation mode.
            ...
            DispatchQueue.main.async {
                self.isHighQualityMode = self.photoOutput.availablePhotoCodecTypes.contains(.hevc)
